...
def workout(user_id):
    ...
    def convert(timing):
        ...
        return converted
    while True:
        workout_option = input("What workout do you want to do? (or go 'back') ").upper()
        workout_name = cf.number_to_workout.get(workout_option)
        print(workout_name)
        if workout_name == "BACK":
            break
        elif workout_name in cf.workout_menu:
